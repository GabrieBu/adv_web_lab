...
import { useQuery } from "@tanstack/react-query";
import Loader from "../loaders/Loader";
import { getStaff, updateOrderStaff, getManage } from "../services/db/staff/apiStaff";
...
function Manage() {
  ...
  useEffect(() => {
    if (!isLoadingManage && manageData) {
      const initialAssignedStaff = manageData.map(item => ({
        orderId: item.id_order,
        staffId: item.id_staff,
      }));
      setAssignedStaff(initialAssignedStaff);
  ...
  const handleStaffAssignment = async (orderId, staffId) => {
    const staffIndex = assignedStaff.findIndex(
      assignment => assignment.orderId === orderId && assignment.staffId === staffId
    );
  ...
  const isStaffAssigned = (orderId, staffId) => assignedStaff.some(
    assignment => assignment.orderId === orderId && assignment.staffId === staffId
  );
    ...
    <div>
      <table style={styles.table}>
        ...
        <tbody>
          {data?.ordersData.map((order) => (
            <tr key={order.id_order}>
              <td style={styles.td}>{order.id_order}</td>
              <td style={styles.td}>
                <select
                  multiple
                  value={assignedStaff.filter(ass => ass.orderId === order.id_order && data?.cookersData.some(cook => cook.id === ass.staffId)).map(ass => ass.staffId)}
                  onChange={(e) => handleStaffAssignment(order.id_order, parseInt(e.target.value))}
                  style={styles.select}
                >
                  {data?.cookersData.map((cook) => (
                    <option
                      ...
                      value={cook.id}
                      style={{
                        color: isStaffAssigned(order.id_order, cook.id) ? "green" : "inherit",
                        ...styles.option
                      }}
                    >
                      {cook.name + " " + cook.surname}
              ...
              <td style={styles.td}>
                <select
                  multiple
                  value={assignedStaff.filter(ass => ass.orderId === order.id_order && data?.waitersData.some(waiter => waiter.id === ass.staffId)).map(ass => ass.staffId)}
                  onChange={(e) => handleStaffAssignment(order.id_order, parseInt(e.target.value))}
                  style={styles.select}
                >
                  {data?.waitersData.map((waiter) => (
                    <option
                      ...
                      value={waiter.id}
                      style={{
                        color: isStaffAssigned(order.id_order, waiter.id) ? "green" : "inherit",
                        ...styles.option
                      }}
                    >
                      {waiter.name + " " + waiter.surname}
